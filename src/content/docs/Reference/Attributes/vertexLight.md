...
The instance light value attribute, equivalent to Iris' [`gl_MultiTexCoord1`](https://shaders.properties/current/reference/attributes/vauv2/). Using `gl_TextureMatrix[1]` converts the range `0.0 - 240.0` to `0.03125 - 0.96875`.

:::note
`clrwl_vertexLight` range is `0.0 - 0.9375`, but `gl_MultiTexCoord1`/`gl_MultiTexCoord2` ranges are `0.0 - 240.0` as multiple shaderpacks rely on that.
:::
